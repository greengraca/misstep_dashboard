/**
 * Seed the EV config for New Phyrexia (nph, May 13 2011).
 *
 * Encodes the 2011 Draft Booster structure documented in:
 *   - Lethe's collation project page for NPH (the print-sheet authority)
 *     https://www.lethe.xyz/mtg/collation/nph.html
 *   - MTG Wiki — confirmed against DB: 175 booster cards = 60 C + 60 U +
 *     35 R + 10 M + 10 basics (5 lands × 2 art).
 *
 * Third (small) set in the Scars of Mirrodin block. The "Phyrexian mana"
 * set with the iconic 5-praetor mythic cycle (Elesh Norn, Sheoldred,
 * Vorinclex, Urabrask, Jin-Gitaxias) plus Karn Liberated, Phyrexian
 * Obliterator, Batterskull, Sword of War and Peace, Etched Monstrosity.
 *
 * Same Draft Booster template as MBS (small set, same era, same foil
 * mechanics). Difference vs MBS: NPH has 60 uncommons (vs MBS 40), so
 * uncommon dilution is higher.
 *
 * Pre-Collector-Booster era. play_booster only.
 *
 * Idempotent — safe to re-run; uses upsert on { set_code: "nph" }.
 */
...
async function buildPools(): Promise<AllPools> {
  ...
  if (all.length === 0) {
    throw new Error(
      `No ${SET_CODE} cards in dashboard_ev_cards. Sync nph from Scryfall before running this seed.`,
    );
  }
  ...
  console.log("NPH pool sanity checks:");
  expect(all.length, 175, "total booster=true (60C + 60U + 35R + 10M + 10 basics)");
  ...
  expect(mythics.length, 10, "mythics (5 praetors + Karn + Phyrexian Obliterator + Batterskull + Sword of W&P + Etched Monstrosity)");

  const layouts = new Set(all.map((c) => c.layout));
  if (layouts.size !== 1 || !layouts.has("normal")) {
    console.warn(`  ⚠️  Unexpected layouts in nph: ${[...layouts].join(", ")}`);
  } else {
...
//   - 10 commons (A run 33×2 = 66 cells, B run 27×2 = 54 cells; per-pack
//     split is half 6A+4B and half 5A+5B)
//   - 3 uncommons (A 33×2 = 66 cells, B 27×2 = 54 cells; "1-or-2 A then
//     1-or-2 B" with 2A in 13/20 of packs)
//   - 1 R/M (35R × 2 + 10M × 1 = 80 cells → P(M) = 1/8)
//   - 1 basic land (10 art prints — 5 lands × 2 art, the same 2-art set
//     as MBS but with NPH-themed metal evolution)
//   - 1 ad / marketing
//
// Foil rate: 15/67 ≈ 22.39% per pack. Sub-distribution 18:12:6:1 (C:U:R:M)
// per the era-typical ratio. Foil replaces a common.
